async function search(country) 
{
  ...
  let response = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=ae249262dcc94ae28ba170541242112&q=${country}&days=3`
  );

  if (response.status == 200 ) 
  {
    let country = await response.json();
    displayCurrentDay(country.location, country.current),
    displayRestDays(country.forecast.forecastday);
  }

}

document.getElementById("search").addEventListener("keyup", (words) => {
  search(words.target.value);
});
